...
export function resolveModelPreferenceToModelKey(preference: string | undefined | null): string | undefined {
...
export function resolvePersonalityPreference(preference: string | undefined | null): PersonalityId | undefined {
  const pref = (typeof preference === 'string' ? preference.trim().toLowerCase() : '') || '';
  if (!pref || pref === 'same') return undefined;

  const valid: PersonalityId[] = ['professional', 'friendly', 'concise', 'creative', 'technical', 'casual'];
