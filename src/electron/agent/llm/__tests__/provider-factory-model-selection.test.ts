...
describe('LLMProviderFactory model selection persistence', () => {
  it('stores selected model in provider-specific fields', () => {
    ...
    const azureSettings: LLMSettings = { providerType: 'azure', modelKey: 'opus-4-5', azure: { deployments: ['existing'] } };
    ...
    expect(LLMProviderFactory.applyModelSelection(openaiSettings, 'gpt-4o').openai?.model).toBe('gpt-4o');
    expect(LLMProviderFactory.applyModelSelection(geminiSettings, 'gemini-2.0-flash').gemini?.model).toBe('gemini-2.0-flash');
    expect(LLMProviderFactory.applyModelSelection(openrouterSettings, 'anthropic/claude-3.5-sonnet').openrouter?.model).toBe('anthropic/claude-3.5-sonnet');
    expect(LLMProviderFactory.applyModelSelection(ollamaSettings, 'llama3.2').ollama?.model).toBe('llama3.2');
    expect(LLMProviderFactory.applyModelSelection(azureSettings, 'new-deployment').azure?.deployment).toBe('new-deployment');
    expect(LLMProviderFactory.applyModelSelection(groqSettings, 'llama-3.3-70b-versatile').groq?.model).toBe('llama-3.3-70b-versatile');
    expect(LLMProviderFactory.applyModelSelection(xaiSettings, 'grok-4').xai?.model).toBe('grok-4');
    expect(LLMProviderFactory.applyModelSelection(kimiSettings, 'kimi-k2.5').kimi?.model).toBe('kimi-k2.5');
