...
import type { AgentMention, CreateMentionRequest, MentionListQuery, MentionType, MentionStatus } from '../../../shared/types';
...
class MockMentionRepository {
  ...
  list(query: MentionListQuery): AgentMention[] {
    ...
    mockMentions.forEach((mention) => {
      ...
      if (query.fromAgentRoleId && mention.fromAgentRoleId !== query.fromAgentRoleId) matches = false;
      if (query.status) {
...
describe('MentionRepository', () => {
  ...
  describe('create', () => {
    ...
    it('should create a mention with optional fields', () => {
      ...
      expect(mention.context).toBe('Please review the changes I made to the authentication module.');
    });
