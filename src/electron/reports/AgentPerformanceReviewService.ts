...
import type { AgentAutonomyLevel, AgentPerformanceReview, AgentReviewGenerateRequest, AgentReviewRating } from '../../shared/types';
...
export class AgentPerformanceReviewService {
  ...
  generate(request: AgentReviewGenerateRequest): AgentPerformanceReview {
    ...
    const completed = taskRows.filter((t) => t.status === 'completed' && typeof t.completed_at === 'number');
    const failed = taskRows.filter((t) => t.status === 'failed');
    const cancelled = taskRows.filter((t) => t.status === 'cancelled');
    const active = taskRows.filter((t) => ['pending', 'queued', 'planning', 'executing', 'paused', 'blocked'].includes(t.status));
    ...
    const mentionCounts = this.db.prepare(`
      SELECT
      ...
      WHERE workspace_id = ?
        ...
        AND created_at >= ?
    `).get(request.workspaceId, request.agentRoleId, periodStart) as { pending: number | null; total: number | null };
    ...
    const activityCountRow = this.db.prepare(`
    ...
    rationaleParts.push(`Rating derived from recent throughput and reliability over the last ${periodDays} day(s).`);
    ...
    summaryLines.push(`Tasks: ${completed.length} completed, ${failed.length} failed, ${cancelled.length} cancelled, ${active.length} active`);
    summaryLines.push(`Mentions: ${pendingMentions}/${totalMentions} pending/total`);
    if (metrics.avgCompletionMinutes > 0) summaryLines.push(`Avg completion: ${metrics.avgCompletionMinutes} min`);
  ...
  delete(reviewId: string): boolean {
    const result = this.db.prepare('DELETE FROM agent_performance_reviews WHERE id = ?').run(reviewId);
