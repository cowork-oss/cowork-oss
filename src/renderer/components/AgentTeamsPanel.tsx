...
export function AgentTeamsPanel({ workspaceId, agents, tasks, onOpenTask }: AgentTeamsPanelProps) {
      ...
      switch (event.type) {
        case 'team_created':
          setTeams((prev) => {
            ...
            const next = existing ? prev.map((t) => (t.id === team.id ? team : t)) : [...prev, team];
            next.sort((a, b) => a.name.localeCompare(b.name));
            return next;
          });
          break;
        case 'team_updated':
          setTeams((prev) => prev.map((t) => (t.id === event.team?.id ? (event.team as AgentTeam) : t)));
          break;
        case 'team_deleted':
        ...
        case 'team_member_added':
          if (event.member?.teamId === selectedTeamId) {
            setTeamMembers((prev) => {
              ...
              const next = existing ? prev.map((m) => (m.id === event.member.id ? event.member : m)) : [...prev, event.member];
        ...
        case 'team_member_updated':
          if (event.member?.teamId === selectedTeamId) {
            setTeamMembers((prev) => prev.map((m) => (m.id === event.member.id ? event.member : m)));
          }
          break;
        case 'team_member_removed':
        ...
        case 'team_item_spawned':
          if (event.item?.teamRunId === selectedRunId) {
            setItems((prev) => prev.map((i) => (i.id === event.item.id ? (event.item as AgentTeamItem) : i)));
          }
          break;
        default:
          break;
  ...
  const handleAddMember = useCallback(async () => {
    ...
    try {
      ...
      setTeamMembers((prev) => {
        ...
        const next = existing ? prev.map((m) => (m.id === member.id ? member : m)) : [...prev, member];
        return next.sort((a, b) => a.memberOrder - b.memberOrder);
      });
      setNewMemberRoleId('');
  ...
  const handleUpdateMember = useCallback(async (memberId: string, updates: Partial<AgentTeamMember>) => {
    try {
      setError(null);
      const updated = await window.electronAPI.updateTeamMember({
        id: memberId,
        ...(updates.isRequired !== undefined ? { isRequired: updates.isRequired } : {}),
        ...(updates.roleGuidance !== undefined ? { roleGuidance: updates.roleGuidance || null } : {}),
  ...
  const handleUpdateRunStatus = useCallback(async (status: 'resume' | 'pause' | 'cancel') => {
  ...
  const handleUpdateItem = useCallback(async (itemId: string, updates: Partial<AgentTeamItem>) => {
    try {
      ...
      const updated = await window.electronAPI.updateTeamItem({
        ...
        ...(updates.ownerAgentRoleId !== undefined ? { ownerAgentRoleId: updates.ownerAgentRoleId || null } : {}),
        ...(updates.sourceTaskId !== undefined ? { sourceTaskId: updates.sourceTaskId || null } : {}),
        ...(updates.status !== undefined ? { status: updates.status as AgentTeamItemStatus } : {}),
        ...(updates.resultSummary !== undefined ? { resultSummary: updates.resultSummary || null } : {}),
  ...
  const handleMoveItem = useCallback(async (item: AgentTeamItem, direction: 'up' | 'down') => {
    ...
    try {
      ...
      setItems((prev) => prev.map((i) => (i.id === a?.id ? (a as AgentTeamItem) : i)).map((i) => (i.id === b?.id ? (b as AgentTeamItem) : i)));
    } catch (err: any) {
              ...
              <div className="mc-form mc-form-grid">
                ...
                <label className="mc-field">
                  ...
                  <select
                    ...
                    onChange={(e) => setTeamDraft({ ...teamDraft, leadAgentRoleId: e.target.value })}
                  >
                    {agents
                      .filter((a) => a.isActive)
                      .map((a) => (
                      ...
                      <div key={m.id} className="mc-row">
                        <div className="mc-row-main">
                          ...
                          <div className="mc-row-sub">
                            {m.isRequired ? <span className="mc-pill info">REQUIRED</span> : <span className="mc-pill neutral">OPTIONAL</span>}
                          </div>
                        </div>
                        <div className="mc-row-actions">
                          <button className="mc-btn tiny" onClick={() => void handleReorderMember(m.id, 'up')}>
                            ↑
                          </button>
                          <button className="mc-btn tiny" onClick={() => void handleReorderMember(m.id, 'down')}>
                            ↓
                          </button>
                          <button
                            className="mc-btn tiny"
                            onClick={() => void handleUpdateMember(m.id, { isRequired: !m.isRequired })}
                          >
                            Toggle required
                          </button>
                          <button
                            className="mc-btn tiny"
                            onClick={() => {
                              const next = prompt('Role guidance (team-specific)', m.roleGuidance || '');
                              if (next === null) return;
                          ...
                          <button className="mc-btn tiny danger" onClick={() => void handleRemoveMember(m)}>
                            Remove
                          </button>
                        </div>
                      </div>
                    ...
                    return (
                      ...
                      >
                        <div className="mc-run-row-top">
                          <span className={`mc-pill ${r.status === 'running' ? 'info' : r.status === 'completed' ? 'success' : r.status === 'failed' ? 'danger' : 'neutral'}`}>
                ...
                {selectedRun && (
                  <div className="mc-section-actions">
                    {(selectedRun.status === 'pending' || selectedRun.status === 'paused') && (
                      <button className="mc-btn primary" onClick={() => void handleUpdateRunStatus('resume')}>
                        {selectedRun.status === 'pending' ? 'Start' : 'Resume'}
                      </button>
                    )}
                    {selectedRun.status === 'running' && (
                      <button className="mc-btn" onClick={() => void handleUpdateRunStatus('pause')}>
                        Pause
                      </button>
                    )}
                    {selectedRun.status !== 'completed' && selectedRun.status !== 'failed' && selectedRun.status !== 'cancelled' && (
                      <button className="mc-btn danger" onClick={() => void handleUpdateRunStatus('cancel')}>
                        Cancel
                      </button>
                    )}
                  </div>
                ...
                <>
                  ...
                  <div className="mc-form mc-form-grid">
                    ...
                    <label className="mc-field">
                      ...
                      >
                        ...
                        {(availableMemberRoles.length > 0 ? availableMemberRoles : agents.filter((a) => a.isActive)).map((a) => (
                          <option key={a.id} value={a.id}>
                            {a.displayName}
                          </option>
                        ))}
                  ...
                  <div className="mc-table">
                    {sortedItems.length === 0 ? (
                      ...
                      sortedItems.map((it) => {
                        const owner = it.ownerAgentRoleId ? agentById.get(it.ownerAgentRoleId) : null;
                        ...
                        return (
                          <div key={it.id} className="mc-row mc-item-row">
                            <div className="mc-row-main">
                              ...
                              <div className="mc-row-sub">
                                <span className={`mc-pill ${itemStatusColorClass(it.status)}`}>{it.status.toUpperCase()}</span>
                                <span className="mc-muted">
                                  Owner: {owner ? `${owner.icon} ${owner.displayName}` : 'Unassigned'}
                                </span>
                                {linkedTask && (
                                  <button
                                    className="mc-link"
                                    onClick={() => (onOpenTask ? onOpenTask(linkedTask.id) : undefined)}
                              ...
                              {it.resultSummary && <div className="mc-row-summary">{it.resultSummary}</div>}
                            </div>
                            <div className="mc-row-actions">
                              <select
                                className="mc-select tiny"
                                value={it.ownerAgentRoleId || ''}
                                onChange={(e) => void handleUpdateItem(it.id, { ownerAgentRoleId: e.target.value || '' })}
                                title="Assign owner"
                              >
                                <option value="">Unassigned</option>
                                {(availableMemberRoles.length > 0 ? availableMemberRoles : agents.filter((a) => a.isActive)).map((a) => (
                              ...
                              <select
                                className="mc-select tiny"
                                value={it.status}
                                onChange={(e) => void handleUpdateItem(it.id, { status: e.target.value as AgentTeamItemStatus })}
                                title="Set status"
                              >
                                {(['todo', 'in_progress', 'blocked', 'done', 'failed'] as AgentTeamItemStatus[]).map((s) => (
                                  <option key={s} value={s}>
                                    {s}
                                  </option>
                                ))}
                              </select>
                              <button className="mc-btn tiny" onClick={() => void handleMoveItem(it, 'up')} title="Move up">
                                ↑
                              </button>
                              <button className="mc-btn tiny" onClick={() => void handleMoveItem(it, 'down')} title="Move down">
                                ↓
                              </button>
                              {it.sourceTaskId && (
                                <button
                              ...
                              <button className="mc-btn tiny danger" onClick={() => void handleDeleteItem(it)}>
                                Delete
                              </button>
                            </div>
                          </div>
